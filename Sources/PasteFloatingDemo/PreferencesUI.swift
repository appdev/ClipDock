...
final class AboutWindowController: NSWindowController {
    ...
    private func makeAppIconView() -> NSView {
        ...
        container.layer?.backgroundColor = NSColor(calibratedRed: 1.0, green: 0.63, blue: 0.12, alpha: 1).cgColor
        container.layer?.cornerRadius = 26
        container.layer?.cornerCurve = .continuous
        container.layer?.shadowColor = NSColor.black.withAlphaComponent(0.26).cgColor
        ...
        imageView.image = preferredAppSymbolImage()
        imageView.contentTintColor = NSColor.white.withAlphaComponent(0.94)
    ...
    private func preferredAppSymbolImage() -> NSImage? {
        let configuration = NSImage.SymbolConfiguration(pointSize: 54, weight: .regular)
        let symbolNames = [
            "clipboard.fill",
            "list.clipboard.fill",
        ...
        for symbolName in symbolNames {
            if let image = NSImage(systemSymbolName: symbolName, accessibilityDescription: "剪贴板工作台") {
