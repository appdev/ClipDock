...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        } else if CommandLine.arguments.contains("--show-preferences") {
    ...
    private func shouldAcceptPanelToggle() -> Bool {
        ...
        guard now - lastPanelToggleUptime > 0.12 else {
            return false
        }
    ...
    private func setItemPinned(_ item: RustClipboardItemSummary, isPinned: Bool) {
        ...
        performItemMutation(.setPinned(itemID: item.id, isPinned: isPinned))
    }
...
extension AppDelegate {
    ...
    func smokeStoredItems() throws -> [RustClipboardItemSummary] {
        guard let appSupportURL else {
            throw NSError(
                domain: "PasteFloatingDemo.RealFunctionQA",
                code: 1,
