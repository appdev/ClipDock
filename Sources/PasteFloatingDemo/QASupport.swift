...
enum PanelInteractionSmokeScenario {
    ...
    private static func verifyFilteringAndSearch(
        ...
        if let imageChip = contentView.smokeTypeFilterButton(itemType: "image") {
            PanelQAHarness.sendMouseDown(to: imageChip, clickCount: 1)
            PanelQAHarness.drainMainRunLoop()
        }
        try PanelQAHarness.require(probe.lastQuery?.itemType == "image", "类型 chip 未触发 image 筛选")
    ...
    private static func verifyManagementMenu(
        ...
        try PanelQAHarness.require(
            probe.pinRequest?.itemID == "panel-smoke-file" && probe.pinRequest?.isPinned == true,
            "固定菜单动作未触发回调"
        )
...
final class PanelInteractionSmokeProbe {
    ...
    private(set) var pinRequest: (itemID: String, isPinned: Bool)?
    ...
    init(controller: FloatingPanelController) {
        controller.onRuntimeAction = { [weak self, weak controller] action in
            ...
            case .copyItem(let item):
                ...
                controller?.hide()
            case .setPinned(let item, let isPinned):
                self?.pinRequest = (item.id, isPinned)
    ...
    func makeReport(
        ...
        PanelInteractionSmokeReport(
            ...
            typeFilter: queries.first { $0.itemType == "image" }?.itemType ?? "none",
            searchText: queries.first { $0.searchText == "report" }?.searchText ?? "none",
            menuPin: pinRequest.map { "\($0.itemID):\($0.isPinned)" } ?? "none",
