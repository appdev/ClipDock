...
- `swift run ClipShelf --render-preferences-snapshot .codex/artifacts/preferences-runtime-snapshot.png`
...
- `swift run ClipShelf --show-context-menu`
- `swift run ClipShelf --show-preview`
- `swift run ClipShelf --show-preview-long`
- `swift run ClipShelf --show-preview-image`
...
- [Sources/ClipShelf/main.swift](/Volumes/extendData/Data/IdeaProjects/Paste/Sources/ClipShelf/main.swift) 已经足够薄，只负责入口和命令分发。
- [Sources/ClipShelf/AppRuntime.swift](/Volumes/extendData/Data/IdeaProjects/Paste/Sources/ClipShelf/AppRuntime.swift) 仍然承载 `FloatingPanelContentView`、`FloatingPanelController`、`AppDelegate` 和大量 UI/系统 glue code。
...
- `demo` 命名仍然出现在 package 名、executable target 名、产物名、打包脚本、release 文档和 QA 命令中，例如 `ClipShelf`。
...
- `Package.swift` 中 `ClipShelf` package / executable 名称
- `.app` 产物名 `ClipShelf.app`
- release zip / dmg 中的 `ClipShelf-*`
...
- `swift run ClipShelf ...` 是当前源码态 QA 命令入口；旧别名不再保留。
...
- Executable target: `ClipShelf`
- QA target: `ClipShelfQATool`
- Tests:
  - `ClipboardDomainTests`
  - `ClipboardAppKitTests`
  - `ClipboardQATests`

如果你们已经有正式品牌名，可以把 `ClipShelf` 替换成品牌名；如果还没有，先用中性产品名比继续留 `Demo` 更合适。
...
- 新建单独的 `ClipShelfQATool` executable target
