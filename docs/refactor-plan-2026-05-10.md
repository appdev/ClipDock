...
- `Sources/PasteFloatingDemo/main.swift` 已经超过 8,000 行，同时承担 UI 组件、窗口控制、偏好设置、剪贴板捕获、菜单栏、快捷键、存储编排、诊断命令、快照命令和 smoke QA。
...
兼容说明：本计划中的 `PasteFloatingDemo` 仅指当前兼容期内保留的可执行 target / 目录名；产品、发布和用户可见命名统一应使用“剪贴板工作台（ClipboardWorkbench）”或中性模块名。
...
兼容期内的 `PasteFloatingDemo` target 拆成以下责任区：
...
- `swift run PasteFloatingDemo --render-panel-snapshot .codex/artifacts/panel-runtime-snapshot.png` 成功
- `swift run PasteFloatingDemo --render-preferences-snapshot .codex/artifacts/preferences-runtime-snapshot.png` 成功
  - 对应 snapshot artifact 需实际落盘
- `swift run PasteFloatingDemo --exercise-panel-interactions` 成功
  - 需在可交互的 GUI 会话中执行
- `swift run PasteFloatingDemo --exercise-preferences` 成功
- 无新增行为变化
- `RustCoreClientTests` 继续全绿

说明：以上验收命令中的 `PasteFloatingDemo` 为当前兼容期内必须保留的旧命令名，用于匹配现有 QA 和脚本入口。
