...
Self-hosted ClipDock sync server v1. This Rust subproject lives under `Server/` in the ClipDock workspace and is separate from the macOS app package.
...
- Local filesystem asset storage with server-side SHA-256 verification, atomic promotion, and sync-space isolation.
...
This v1 server is for self-hosted sync. Run it on localhost, a private network, or behind an HTTPS reverse proxy for real use. Do not expose it directly over the public internet without TLS, request-size limits, backups, and normal host hardening.
