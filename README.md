...
- 偏好设置：修改默认高度、历史规则、内容类型、忽略列表和外观选项会立即保存；默认高度会影响面板高度，关闭图片记录后不再捕获图片剪贴板，开启文件记录后会捕获文件 URL 剪贴板；命中应用标识、窗口标题关键词或未知来源跳过规则时，文本/图片/文件捕获会在写资产和入库前停止；窗口标题采集为 best-effort，系统不返回标题时不会影响应用标识和未知来源规则；关闭预览浮层后 `Space` 不再展示预览；保存数量和保留天数会在启动、捕获和偏好保存后自动软清理普通历史。
...
- 本地维护：每次启动会自动整理本地库；只清理已软删除条目关联资产、未被数据库引用的 `assets`/`thumbnails` 文件和 `staging` 残留，不清理来源应用图标。
