...
![ClipDock 当前剪贴坞](assets/marketing/clipdock-panel-overview-screen-real.webp)
...
![ClipDock 预览浮层](assets/marketing/clipdock-preview-screen-real.webp)

![ClipDock Pinboard 筛选](assets/marketing/clipdock-panel-pinboard-screen-real.webp)
...
- **一次性配对码**：通过 `POST /v1/sync/create` 创建同步空间，再用短期 5 位配对码让新设备加入；设备 token 只以哈希形式保存在服务端。
...
- `Server/`：自托管同步服务端，协议文档在 `Server/docs/protocol-v1.md`。
- `Android/`：Android 端，包含同步空间配置、快照/事件拉取、P2P 下载和悬浮球入口。
- `index.html`、`assets/`、`open-clipdock.html`：产品官网与安装帮助页，已提升到项目根目录。
...
服务端部署边界和 API 细节见 [Server/README.md](Server/README.md) 与 [Server/docs/protocol-v1.md](Server/docs/protocol-v1.md)。
